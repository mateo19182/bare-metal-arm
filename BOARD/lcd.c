...
void lcd_set2(char value, uint8_t digit)
{
    if (digit > 4) {
    ...
    switch (value) {
        ...
        case '9':
            ...
            break;
        case '.':
            // Setting only the G segment to display a minus sign
            LCD->WF8B[LCD_Frontplane_Pin[(2 * digit) - 2]] = LCD_CLEAR; // Clear the other segments
            LCD->WF8B[LCD_Frontplane_Pin[(2 * digit) - 1]] = LCD_CLEAR;
            break;
        case '-':
            // Setting only the G segment to display a minus sign
            LCD->WF8B[LCD_Frontplane_Pin[(2 * digit) - 2]] = LCD_SEG_G; // Clear the other segments
            //LCD->WF8B[LCD_Frontplane_Pin[(2 * digit) - 1]] = LCD_SEG_F;
            break;
...
void lcd_display_dec2(int16_t value)
{
    if (value > 9999 || value < -999) {
        // Display "Err" if value has more than 4 digits or less than -999
        lcd_display_error(0x10);
    } else {
        // Determine the number of digits to shift the display based on sign
        uint16_t pos_value = (value < 0) ? -value : value;  // Convert to positive if negative

        // Clear previous content (Assuming lcd_set2 is used to handle digits and positions)
        ...
        if (value < 0) {
            ...
            lcd_set2('0' + (pos_value / 1000), 1);         // Thousands place
        }
        // Display digits
        lcd_set2('0' + ((pos_value % 1000) / 100), 2); // Hundreds place
        lcd_set2('0' + ((pos_value % 100) / 10), 3);   // Tens place
        lcd_set2('0' + (pos_value % 10), 4);           // Units place
    }
}
